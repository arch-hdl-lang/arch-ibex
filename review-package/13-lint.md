...
## 3a. Re-run on the released arch 0.72.0 (TASK3 A5, 2026-09-05)

Same command, same file lists, `build/` regenerated by the release binary (which emits
byte-identical SV to the TASK2 pin): **141 warnings with waivers, 490 without — every
class count identical to §1**, 0 errors. The `PROCASSINIT` pattern of §3
(arch-com#995) is still emitted, 2 occurrences in `build/ibex_multdiv_fast.sv`
(`_t0_state`, `_t0_loop_cnt_0`); no bin (ii) pattern changed. Logs:
`reports/lint_v0720_arch_lane_ibex_top.log`, `reports/lint_v0720_arch_lane_ibex_top_nowaiver.log`.
