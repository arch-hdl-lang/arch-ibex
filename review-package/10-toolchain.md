...
|---|---|---|---|
| ARCH compiler | 0.72.2 (release asset, see decisions table; SV identical to 0.72.1, on which synthesis / P&R / ECP5 / lint were run) | `~/.local/arch-v0.72.2/arch-aarch64-apple-darwin/arch` (symlink `~/.local/bin/arch-0.72.2`) | everything from TASK3 on |
...
| Decision | Choice |
|---|---|
| ARCH compiler pin (current, since 2026-09-20) | **arch `0.72.5` from the published release** (https://github.com/arch-hdl-lang/arch-com/releases/tag/v0.72.5; asset `arch-aarch64-apple-darwin.tar.xz`, 2,465,308 bytes, SHA-256 `97f609746d4e951373f0dc3bd207ea5bd08e0a17276806ee2e475ae6d14af85e`, verified against the release's own `.sha256` before install; unpacked to `~/.local/arch-v0.72.5/…`; `arch --version` → `arch 0.72.5 (git 64830b222bce)`; `.arch-version` = `0.72.5`). **The port's generated SV is byte-identical to the 0.72.4 build — all 23 files** (`cmp` per file against a freshly rebuilt 0.72.4 baseline, plus a whole-tree digest of the sorted per-file hashes: `759135eafb43024dc2e6c649e4acb2c4` for both). v0.72.5's emitter fixes (concatenation sizing, signedness and others in its release notes) therefore change nothing in this design, so **every gate, synthesis, P&R, ECP5 and lint number measured on 0.72.4 stands unchanged** and neither flow was re-run (TASK11B D1). |
| Previous pin (2026-09-20, TASK9) | **arch `0.72.4`** (asset SHA-256 `263ed10d28bcf170db4664fed4c106ab78cc8babc3a7c1bc3b5ce97eee8818bd`). Moved from 0.72.2 to pick up PR #1028, which restores the `shared function` harness that arch-com #247 had silently disabled since 2026-05-11 — `ibex_multdiv_fast` was emitting four multipliers where one was intended (`16-per-module.md` §6a). Generated SV vs the 0.72.2 build: **3 of 23 files changed**, all explained — `ibex_multdiv_fast.sv` (94 differing lines, #1028's shared MAC), `ibex_load_store_unit.sv` (16) and `ibex_compressed_decoder.sv` (6), the latter two from PR #1014's sext-signedness fix (`$signed(...)` wrapping of sign-extending concatenations) which shipped in v0.72.3. The other 20 files are byte-identical. Numbers re-measured on 0.72.4: per-module attribution (`16-per-module.md`), sky130 post-P&R (`14-sky130.md` Table 10) and ECP5 post-P&R (`15-ecp5.md` §5.6). *Note: this row was added retrospectively in TASK11B — TASK9 moved `.arch-version` and documented the change in `16-per-module.md` but did not update this table.* |
| Pin during the TASK2/TASK3 measurements | **arch `0.72.2` from the published release** (https://github.com/arch-hdl-lang/arch-com/releases/tag/v0.72.2, tag on merge commit `3a3744e0`; asset `arch-aarch64-apple-darwin.tar.xz`, 2,462,740 bytes, SHA-256 `e4eb0952f5adc8af9eb22983251219aef2b6b5e2da75da9264a398e78bcd9b5c`, unpacked to `~/.local/arch-v0.72.2/…`; `.arch-version` = `0.72.2`). 0.72.2 = 0.72.1 + PR #1005 (native simulator: module-scope lets evaluated after the comb blocks, closes #1003) — a simulator-backend fix only: **all 23 generated SV files are byte-identical to the 0.72.1 build in both profiles**, so every synthesis, P&R, ECP5 and lint number below measured on 0.72.1 stands unchanged; only the gate was re-run (`reports/gate_v0722_*`, `arch_check_v0722.log`): 168 passed / 6 failed / 75 skipped, all six `archsim` suites green. Previous: 0.72.1 (tag on `0ea5cbd0`; asset SHA-256 `f3cfcd606ca71e59f0ac09e5bf28ac10ce1452b48dc19f961eefd30fff987170`; `.arch-version` was `0.72.1`). 0.72.1 = 0.72.0 + PR #1001 (no compiler-introduced declaration initializers, closes #995) + PR #1002 (`arch sim --pybind` Vec-port binding, closes #996), both found by this package. Its generated SV differs from 0.72.0 only in `ibex_multdiv_fast.sv` (three `= 0` declaration initializers gone, the two thread counters in the reset branch). Numbers re-run on 0.72.1: the gate, the core lint, sky130 synthesis and ECP5 (see the TASK3 Part A table below and `14-sky130.md` §4.6, `15-ecp5.md` §5.5); post-P&R re-run on 0.72.1 as well (`14-sky130.md` §4.6; the 0.72.0 reports are kept as `arch_openroad_v0720_final_*`). Previous pin: | **arch `0.72.0` from the published release** (https://github.com/arch-hdl-lang/arch-com/releases/tag/v0.72.0, tag on merge commit `e5e93bf8`, 2026-09-05). Installed asset: `arch-aarch64-apple-darwin.tar.xz` (2,462,548 bytes), SHA-256 `a6163bacfbd892c1cb7751382bf5cd186f9f22ff380a72fc33dcb83896de1586` (matches the release's `sha256.sum`), unpacked to `~/.local/arch-v0.72.0/arch-aarch64-apple-darwin/arch`, `arch --version` → `arch 0.72.0`. The repo now pins this version in `.arch-version`; `scripts/build.sh` refuses any other `arch` (TASK3 A1). Install on another machine: download the platform asset from the release page (or run its `arch-installer.sh`), verify the SHA-256 against `sha256.sum`, set `ARCH_BIN`. History: the TASK2 measurements (Phases 2–5) were produced with the local merge commit `89ec0522` = main `f4569890` + PR #993 + PR #994, whose two patches are kept under `attic/` for provenance only (both PRs are merged; nothing needs `git am` any more); rebuilding the port with v0.72.0 emits byte-identical SV for all 23 files (`diff -r`, 2026-09-05), and the TASK3 re-gate below reproduces every TASK2 number. |
...
### Re-gate on arch 0.72.2 (2026-09-05, late evening)

0.72.2 ships only the native-simulator fix for arch-com#1003 (PR #1005). `make build` /
`make build-synth` 23 / 23; **every generated file is byte-identical to the 0.72.1
build** (`cmp` over both profiles: 0 differences), `arch check` 23 / 23.

| Suite (assertions on) | 0.72.1 | **0.72.2** | Files |
|---|---|---|---|
| `make lint` | FAIL, 1 warning | FAIL, 1 warning (`SYNCASYNCNET`) | `reports/gate_v0722_make_lint.*` |
| `make test` (249 items) | 167 / 7 / 75 | **168 passed / 6 failed / 75 skipped** | `reports/gate_v0722_make_test.*` |
| `test_arch_tests.py` | 74 / 74 | 74 / 74 | `reports/gate_v0722_arch_tests.*` |
| CoreMark compare | 1.0108 | 1.0108 (112,855 vs 111,651 ticks, validated) | `reports/gate_v0722_coremark.*` |

The 6 remaining failures are all outside the compiler and the design: the four HARC
canaries and the never-committed plan file (repo/tool drift as before) and
`test_soc_lint` (the `SYNCASYNCNET`). `test_archsim_units` is 6 / 6 — the compressed
decoder's cocotb suite is 29 / 29 under `arch sim --pybind`, the last native-simulator
divergence (#1003) being fixed.
