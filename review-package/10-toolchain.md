...
| Decision | Choice |
|---|---|
| ARCH compiler pin | **v0.71.0 + one compiler fix**: arch-com `ead3aa8f164cc4a33e87a53f358286237b3bf878` = tag `v0.71.0` (`1a7d9fd7`, 2026-07-26) + `reports/arch_com_v0.71.0_stub_variant_fix.patch` (interface stubs exempt from variant-name mangling). `arch --version` → `arch 0.71.0`. Chosen at the Phase 1 gate (option A in `11-port-changes.md`). The `IbexIcache.arch` operand-alignment error is handled by a source change (`11-port-changes.md` Change 1). Upstream PR for the fix: see "arch-com PR" row below once opened. |
