...
| Decision | Choice |
|---|---|
| ARCH compiler pin (final, from Phase 4 on) | **arch-com main `f4569890` (2026-09-01) + PR #993 + PR #994** = local merge commit `89ec0522aa8e99ca5f875175a3fef59289e47e88` (parents `f4569890`, `a814dc62`, `7844c021`). `arch --version` → `arch 0.71.0` (the version string on main). Rebuild: `git checkout f4569890 && git am reports/arch_com_pin_0001-*.patch reports/arch_com_pin_0002-*.patch && cargo build --release`. Both PRs are CI-green and pending merge; once merged, plain `main` is the pin. |
