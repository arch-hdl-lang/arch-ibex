...
#      SYNTH_NOABC (default 0; experiment only, see below).
#
...
# SYNTH_NOABC=1 (off by default): run `synth` with -noabc, so the logic is
# restructured once, by the liberty `abc` below, instead of first by synth's
# generic ABC pass. Measured depth flop->data_addr_o[31]: arch 39->36, sv 30->32.
# (`abc -D <ps>` on top of this changed neither depth, so it is not offered.)
# Both lanes get the same setting when both run; the default script is unchanged.
SYNTH_NOABC="${SYNTH_NOABC:-0}"
SYNTH_OPTS=""
if [ "$SYNTH_NOABC" = "1" ]; then SYNTH_OPTS=" -noabc"; fi
