"""D2-followup: PMP execute-fault end-to-end on the swapped Ibex SoC.

Tests that the IF-stage + IbexPmp + cs_registers + controller chain
correctly delivers a synchronous instruction-access-fault when an
M-mode fetch hits a PMP-locked, exec-denied region.

Released reset → Ibex executes `_start` from 0x0010_0100, which:
  1. Installs `mtvec` at trap_entry.
  2. Programs PMP region 0 as NA4-locked covering `forbidden_func`,
     with X=W=R=0 (cfg byte = 0x88).
  3. `jal x0, forbidden_func` — fetch from forbidden_func MUST fault
     before the first instruction there commits.
...
Asserts:
  ...
  * `saved_mepc   == &forbidden_func`
                                     — faulting instruction's PC.
  * `saved_mtval  == &forbidden_func`
                                     — faulting fetch address.

Why it exists: D2 flipped PMPEnable=1 but no existing ISR program
exercises a PMP violation. This is the smallest meaningful program
that wires IbexPmp into a real fault delivery path.
"""
...
# `tests/sw/build/pmp_exec_isr.dis`.
FORBIDDEN_FUNC  = 0x0010_0148
...
@cocotb.test()
async def pmp_exec_fault_delivers_to_handler(dut) -> None:
    ...
    for _ in range(5000):
        ...
        raise AssertionError(
            f"PMP exec-fault program never completed after 5000 cycles; "
    ...
    assert saved_mcause == 1, (
        ...
        f"forbidden_func={FORBIDDEN_FUNC:#x}"
    )

    # mepc carries the faulting instruction's PC (the fetch that hit
    # the PMP-locked region — i.e. forbidden_func itself).
    assert saved_mepc == FORBIDDEN_FUNC, (
        f"mepc: expected forbidden_func {FORBIDDEN_FUNC:#x}, "
        f"got {saved_mepc:#x}"
    )

    # mtval mirrors the faulting fetch address for instruction access
    # faults (Privileged Spec Vol II §3.1.16).
    assert saved_mtval == FORBIDDEN_FUNC, (
        f"mtval: expected fetch addr {FORBIDDEN_FUNC:#x}, "
        f"got {saved_mtval:#x}"
    )
